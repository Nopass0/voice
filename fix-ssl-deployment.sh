#!/bin/bash

# This script fixes SSL deployment issues on the server

echo "Fixing SSL deployment on server..."

# Server details
SERVER_HOST="194.58.105.224"
SERVER_USER="root"
SERVER_PORT="22"

# Create a temporary script to run on the server
cat > /tmp/fix-ssl-server.sh << 'EOF'
#!/bin/bash

# Ensure SSL directory exists
mkdir -p /opt/voice/ssl

# Create the private key file
cat > /opt/voice/ssl/private.key << 'PRIVKEY'
-----BEGIN RSA PRIVATE KEY-----

Jp9MrqSyKRJLyU6R0kNlz2vfD7G5fF4h6vHQ6n7qN0eV/e8kNQ8y
-----END RSA PRIVATE KEY-----
PRIVKEY

# Set correct permissions
chmod 600 /opt/voice/ssl/private.key
chown root:root /opt/voice/ssl/private.key

# Verify the file
echo "Checking SSL files:"
ls -la /opt/voice/ssl/

# Restart services
cd /opt/voice
docker-compose down
docker-compose up -d

# Wait for services to start
sleep 15

# Run database migrations
echo "Running database migrations..."
docker-compose exec -T backend bun run prisma db push || {
  echo "Failed to run migrations, retrying..."
  sleep 10
  docker-compose exec -T backend bun run prisma db push || echo "Migration failed, manual intervention may be required"
}

echo "SSL deployment fixed!"
EOF

echo "Instructions to fix SSL deployment:"
echo "1. Copy the script to the server:"
echo "   scp /tmp/fix-ssl-server.sh root@194.58.105.224:/tmp/"
echo ""
echo "2. SSH to the server and run:"
echo "   ssh root@194.58.105.224"
echo "   chmod +x /tmp/fix-ssl-server.sh"
echo "   /tmp/fix-ssl-server.sh"
echo ""
echo "Or run this command to do it all at once:"
echo "cat /tmp/fix-ssl-server.sh | ssh root@194.58.105.224 'cat > /tmp/fix-ssl.sh && chmod +x /tmp/fix-ssl.sh && /tmp/fix-ssl.sh'"